...
## Подяка тим, хто це збудував

Перш за все: цей хендбук існує тому, що складну частину зробив хтось інший.

Власний `README.md` SEMS називає двох людей. **Raphael Coeffic** (rco@iptel.org) — *«the father
of SEMS»* — написав цю річ, включно з SIP-стеком у `core/sip/`, яким
[частина 3](07-sip-stack-overview.md) захоплюється п'ять розділів поспіль. **Stefan Sayer** —
*«the lead developer»* — вів проєкт роками й написав фреймворк `apps/sbc`, на який
[частині 6](23-sbc.md) потрібно три розділи.

Повний список контриб'юторів живе в
[README проєкту](https://github.com/sems-server/sems#authors), де йому й місце — це книга про
код, а не дошка пошани, і саме рішення цих двох людей ви читатимете майже на кожній сторінці.

README дякує також **FRAFOS GmbH, sipwise GmbH, IPTEGO GmbH, iptelorg GmbH і TelTech Systems
Inc.** за спонсорування розробки. Двоє з цих імен повернуться в
[частині 12](43-family-overview.md), бо «проспонсувати проєкт, а потім повести його своїм
шляхом» — це значною мірою і є історія того, як у цієї родини з'явилось кілька гілок.

> [!NOTE]
> Там, де цей хендбук вказує на щось дивне — `sleep(5)` у прибиральнику сесій
> ([2.3](04-memory-and-ownership.md)), порівняння `cstring`, що звіряє лише коротшу довжину
> ([3.3](09-parser.md)), пулова модель сесій, викомпільована й лишена на місці
> ([2.1](02-thread-model.md)) — читайте це як археологію, а не як критику. Це двадцять років
> продакшн-медіасервера, крізь який увесь цей час гнали справжній трафік. Книгу пише той, хто
> читає код, а не той, кому треба було змусити це працювати у 2004-му.
...
довідкові матеріали, зокрема [мапа термінів Kamailio↔SEMS](52-term-map.md), яку варто прогорнути
